...
async function getOverdueKegsV2(db) {
  ...
  // "One person, one job" (per the business's own setup) - each
  // operational role maps to exactly one active user, so this is a
  // simple role->name lookup, not a real assignment mechanism. Built
  // once here rather than queried per-alert.
  const { rows: userRows } = await db.query(`SELECT role, name FROM users WHERE active = true`);
  const userNameByRole = {};
  for (const u of userRows) userNameByRole[u.role] = u.name;
  ...
  for (const row of rows) {
    if (row.pending_handover_to) {
      ...
      overdue.push({
        ...
        phaseType: 'pending',
        receivingLocation: row.pending_handover_to,
        receivingUserName: userNameByRole[def.role] || null,
