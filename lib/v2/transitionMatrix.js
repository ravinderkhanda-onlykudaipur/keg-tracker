...
const TRANSITION_MATRIX = [
  ...
  // ===================================================================
  {
    ...
    // Genuinely the one point a keg's cycle resets - the previous
    // customer/product only become stale here, since every other
    // 'empty' state along the way (mover_holding_empty,
    // washer awaiting wash, etc.) is still part of the SAME cycle
    // that hasn't been re-filled/re-delivered yet. Without this,
    // current_customer_id/current_product_id persist indefinitely
    // across reuses of the keg, showing the previous delivery's
    // customer/product long after they've stopped being relevant.
    clearsCustomerAndProduct: true,
  },
  {
    ...
    // Gated by an SMS OTP sent to the customer's own phone via
    // MSG91's own OTP API (generation, sending, AND verification all
    // delegated to MSG91 - see lib/msg91.js) - the frontend shows a
    // "Send Code" + code-entry flow instead of the normal single-step
    // form for this transition specifically, and the actual
    // completion happens via /verify-delivery-otp rather than the
    // regular /execute endpoint. See lib/msg91.js and the
    // /send-delivery-otp, /verify-delivery-otp routes in
    // routes/v2Kegs.js for the full flow.
  ...
  // ===================================================================
  {
    ...
    // Mover<->Warehouse transitions are single-actor, not two-scan -
    // both entities are operated by the same physical person (the
    // 'warehouse' DB role, displayed as "Mover" - see
    // entityRoleMapping.js), so requiring a separate confirming scan
    // for someone to confirm their own internal warehouse movement is
    // pure friction with no real custody-verification value, unlike a
    // genuine handoff between two different people.
    sender: 'mover', receiver: 'warehouse', twoScan: false,
    from: { location: 'mover',     condition: 'empty' },
  ...
  },
  {
    ...
    sender: 'warehouse', receiver: 'mover', twoScan: false,
  ...
  },
  {
    ...
    // v1's fill action captured product/batch/ABV; v2's original
    // version of this transition didn't capture anything at all about
    // what was actually filled - a real functional regression, not
    // just a missing nice-to-have, found during a full audit rather
    // than reported as a bug first. Volume is fixed at 20L per fill,
    // same as v1 - not asked for, just recorded.
  ...
  },
  {
    ...
    // 'damaged'. Resolving it from there is Mover's own explicit
    // choice - see mover_repairs_damaged_keg below - not modeled as an
    // automatic follow-on step.
    to:   { location: 'mover', condition: 'damaged' },
  },
  // Mover's explicit resolution once a damaged keg is back in their own
  // hands - single-actor (no second party to confirm; it's a judgment
  // call Mover makes alone), reintroducing it to the normal cycle at
  // exactly the same location/condition a routinely-returned empty keg
  // would have, so it goes through a full wash + inspection again
  // rather than skipping any step just because it was previously
  // damaged. This is the resolution the spec's own "further authorized
  // action" phrasing pointed at without specifying it outright -
  // permanently retiring a keg instead (rather than repairing it) is a
  // distinct, larger decision - it needs its own lifecycle concept
  // (active/inactive) separate from location+condition, which isn't
  // modeled yet and is deliberately left for a dedicated follow-up
  // rather than folded into this transition.
  {
    id: 'mover_repairs_damaged_keg',
    label: 'Mark repaired - return to cycle',
    sender: 'mover', receiver: 'mover', twoScan: false,
    from: { location: 'mover', condition: 'damaged' },
    to:   { location: 'mover', condition: 'empty' },
  },
