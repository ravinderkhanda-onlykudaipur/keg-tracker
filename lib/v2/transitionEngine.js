...
function executeSingleActor(keg, transitionId, actingDbRole, details, otpVerified = false) {
  const t = TRANSITION_MATRIX.find((x) => x.id === transitionId);
  if (!t) return { ok: false, error: `Unknown transition: ${transitionId}` };
  if (t.twoScan) return { ok: false, error: `${transitionId} is a two-scan transition - use initiateHandover/confirmHandover instead` };
  // Critical gate: a transition flagged requiresOtpVerification can
  // ONLY execute when explicitly called with otpVerified=true, which
  // only routes/v2Kegs.js's verify-delivery-otp endpoint ever passes,
  // and only after it has independently confirmed the submitted code
  // matches. Without this check here (at the engine level, not just
  // in one route), the transition would still be fully executable via
  // the completely ordinary POST /:id/execute endpoint - selecting it
  // from the normal action dropdown and submitting would complete the
  // "OTP-gated" delivery with zero code ever sent or verified,
  // silently defeating the entire feature. Confirmed this exact bypass
  // worked before this check was added, by calling executeSingleActor
  // directly with no otpVerified argument at all.
  if (t.requiresOtpVerification && !otpVerified) {
    return { ok: false, error: `${transitionId} requires OTP verification - use the delivery confirmation code flow, not this endpoint directly` };
  }
  if (!dbRoleCanActAsEntity(actingDbRole, t.sender)) {
    return { ok: false, error: `This action requires acting on behalf of '${t.sender}' - your role can't do that` };
  }
