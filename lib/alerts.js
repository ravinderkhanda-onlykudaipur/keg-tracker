...
const ALERT_RULES = {
  ...
  // Mirrors empty_returned above, for the clean-stock side: Mover
  // hasn't yet confirmed receiving the keg Washer sent their way.
  clean_storage:          { hours: () => getHoursEnv('ALERT_RECEIVE_CLEAN_HOURS', 6), role: 'warehouse', label: 'not received yet' },
  received_from_washer:   { hours: () => getHoursEnv('ALERT_ALLOT_FILL_HOURS', 24),   role: 'warehouse', label: 'not allotted to filler yet' },
  // Mover hasn't yet confirmed receiving the completed fill from Filler.
  filled:                 { hours: () => getHoursEnv('ALERT_RECEIVE_FILLED_HOURS', 6), role: 'warehouse', label: 'not received yet' },
  received_from_filler:   { hours: () => getHoursEnv('ALERT_DISPATCH_HOURS', 120),    role: 'warehouse', label: 'not dispatched yet' },
  ...
  // this phase taking too long. Label now specifically means "hasn't
  // physically returned it to the warehouse yet" - a separate step
  // from Mover confirming receipt (see returned_to_warehouse below).
  empty_at_customer:     { hours: () => getHoursEnv('ALERT_PICKUP_HOURS', 24),      role: 'driver',    label: 'pending at driver, not yet returned' },
  // Driver has brought the keg to the warehouse and logged it (their
  // own "sender-side" scan), but Mover hasn't yet confirmed receiving
  // it - short-fuse, matching every other receive step.
  returned_to_warehouse: { hours: () => getHoursEnv('ALERT_RECEIVE_RETURN_HOURS', 6), role: 'warehouse', label: 'not received yet' },
