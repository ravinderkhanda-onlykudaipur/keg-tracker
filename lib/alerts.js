...
const ALERT_RULES = {
  ...
  // allotted). The "washer hasn't washed it" alert moved to the new
  // allotted_washer status, which is when it's actually their turn.
  empty_returned:    { hours: () => getHoursEnv('ALERT_ALLOT_WASH_HOURS', 24), role: 'warehouse', label: 'not allotted to washer yet' },
  allotted_washer:   { hours: () => getHoursEnv('ALERT_WASH_HOURS', 48),      role: 'washer',    label: 'not washed yet' },
  ...
  empty_at_customer: { hours: () => getHoursEnv('ALERT_PICKUP_HOURS', 24),    role: 'warehouse', label: 'empty, awaiting pickup' },
