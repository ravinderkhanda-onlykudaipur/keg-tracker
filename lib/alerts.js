...
// Returns an array of overdue kegs: [{ kegId, status, role, label,
// statusSince, hoursInStatus, hoursOverdue }, ...]. `db` is passed in
// (not required here) to keep this easy to unit test with a fake db.
function getOverdueKegs(db) {
  const rows = db.prepare(`
    SELECT k.id, k.status, k.created_at AS keg_created_at,
  ...
  for (const row of rows) {
    ...
    const statusSinceStr = row.last_event_at || row.keg_created_at;
    const hoursInStatus = (now - parseSqliteUtc(statusSinceStr).getTime()) / (1000 * 60 * 60);
