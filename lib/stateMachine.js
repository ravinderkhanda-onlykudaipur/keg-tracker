...
const TRANSITIONS = {
  ...
  // touching the keg's status or requiring a full revert - `to: null`
  // means "no status change" (same pattern validateTransition already
  // supports for exactly this case). Scoped to `received_driver` only
  // - once Driver has actually confirmed physical custody, but before
  // delivery has happened. Deliberately excludes plain `dispatched`:
  // that status means the keg is only notified/assigned, not yet
  // confirmed in Driver's hands, so editing at that point isn't the
  // intended window.
  edit_destination:    { role: 'warehouse', from: ['received_driver'],                   to: null },
