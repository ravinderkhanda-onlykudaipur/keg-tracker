...
const TRANSITIONS = {
  ...
  // Mover's choice at the moment of receiving a returned keg: put it in
  // Uncleaned storage (the default, unchanged from before), or send it
  // straight to Washer, skipping the storage stop entirely. Mirrors the
  // wash action's next_stop branch above, same reasoning: Mover decides
  // in the moment rather than it always being a separate later step.
  receive_empty:       { role: 'warehouse', from: ['empty_at_customer'],
