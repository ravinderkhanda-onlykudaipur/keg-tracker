...
async function sendDeliveryOtpMsg91(phone) {
  ...
  // Diagnostic logging - added specifically because a persistent 403
  // "Invalid request" only reproduced from the production server
  // (never from a manual curl test with the same logical values) went
  // unexplained even after fixing the actual credential values on
  // Render. Only the RESPONSE was ever logged before this - the
  // actual REQUEST our code sends was never directly verified, so a
  // subtle difference (e.g. in how the phone number normalizes) could
  // not have been ruled out. Deliberately logs the authkey's length
  // and first/last few characters only, never its full value, and
  // never logs the phone number itself.
  console.log('[msg91] sendOtp request:', JSON.stringify({
    url: `${WIDGET_BASE_URL}/sendOtp`,
    widgetId,
    identifier,
    authkeyLength: authkey ? authkey.length : 0,
    authkeyPreview: authkey ? `${authkey.slice(0, 4)}...${authkey.slice(-4)}` : null,
  }));

  // authkey sent BOTH as a header AND inside the JSON body. The
  // Send OTP/Verify OTP docs pages showed authkey as a header, but
  // MSG91's own "Server Side Integration" example (for the related
  // verifyAccessToken endpoint) showed it inside the request body
  // instead - given a real, persistent "Authentication failure" even
  // after trying both the widget token and the account authkey as
  // the header value, sending it both ways is the safe, defensive
  // fix: whichever MSG91 actually checks for this endpoint, the
  // request is correctly authenticated either way.
  //
  // user-agent added explicitly: a real, reproduced 403 "Invalid
  // request" happened ONLY from this server, never from an identical
  // manual curl test with confirmed-matching widgetId/identifier/
  // authkey values (verified directly via request logging - every
  // field matched exactly). curl sends its own User-Agent by default;
  // Node's fetch() sends none - given MSG91's API sits behind
  // Cloudflare (visible in their response headers), a missing
  // User-Agent is a common bot-detection trigger this could plausibly
  // be hitting.
  const res = await fetch(`${WIDGET_BASE_URL}/sendOtp`, {
    ...
    headers: {
      ...
      'user-agent': 'KegTracker/1.0 (+https://keg-tracker.onrender.com)',
    },
    body: JSON.stringify({ widgetId, identifier, authkey }),
  });
...
async function verifyDeliveryOtpMsg91(reqId, submittedCode) {
  ...
  try {
    res = await fetch(`${WIDGET_BASE_URL}/verifyOtp`, {
      ...
      body: JSON.stringify({ widgetId, reqId, otp: String(submittedCode), authkey }),
    });
