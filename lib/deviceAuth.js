...
//
// Admin can also pause this gating entirely (app_settings key
// 'device_approval_paused') - meant for active testing, where
// constantly re-approving devices as people switch roles/phones is
// pure friction with no real security benefit yet. While paused, every
// device is allowed through for every gated role, with nothing logged
// or registered - flipping the pause back off returns to exactly the
// per-user approval behavior described above, unaffected by whatever
// devices were used while paused.
...
async function checkAndRegisterDevice(db, role, deviceId, userId) {
  ...
  const paused = await getSetting(db, 'device_approval_paused', 'false');
  if (paused === 'true') return { allowed: true };
